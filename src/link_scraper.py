...
class LinkScraper:
    def __init__(self, root):
        ...
        self.base_path = ""  # Store the base path for section crawling
    ...
    def create_widgets(self):
        ...
        instructions_text = (
            ...
            "2. Select scraping mode. See !info document for guidance.\n"
            "3. Paste a website link (URL).\n"
        ...
        ttk.Radiobutton(
            mode_frame, 
            text="Section Crawl: Get links within multiple pages from a wider sector of a website. Be careful!", 
            variable=self.scrape_mode, 
            value="domain"
        ).pack(anchor=tk.W, pady=2)
    ...
    def start_scraping(self):
        ...
        # Extract the base path for section crawling
    ...
    def is_in_base_path(self, url):
        """Check if a URL is within the base path section of the website."""
        parsed = urlparse(url)
        path = parsed.path
        ...
        # URL is in base path if its path starts with the base path
        ...
        try:
            ...
            for a_tag in soup.find_all('a', href=True):
                ...
                if normalized_url not in self.links_found:
                    ...
                    # For section crawl, add new URLs to the queue if they're in the base path
                    if self.scrape_mode.get() == "domain":
                        link_domain = self.get_domain(normalized_url)
                        if (link_domain == base_domain and 
                            normalized_url not in self.visited and
                            self.is_in_base_path(normalized_url)):
